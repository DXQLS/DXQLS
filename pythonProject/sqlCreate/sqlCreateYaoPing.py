        ...
        try:
            ...
            for sheet_name, sheet_data in excel_data.items():
                ...
                for index, row in sheet_data.iterrows():
                    ...
                    medicine_component = row.get("medicine_component")
                    remark = row.get("remark")
                    bas_deduction_rule_id = row.get("bas_deduction_rule_id")
                    medicine_type = row.get("medicine_type")

                    values = (
                        item_name, item_code, common_name, dosage_form, fee_level, common_ratio, medicine_component,
                        remark, bas_deduction_rule_id, medicine_type)
                    query = f"INSERT INTO `{database_name}`.`{table_name}` (item_name, item_code, common_name, dosage_form, fee_level, common_ratio, medicine_component, remark, bas_deduction_rule_id, medicine_type) VALUES {values}"

                    sql_statements.append(query)

            # 生成SQL文件
            sql_file_name = f"{database_name}_{table_name}_yaopin_insert.sql"
