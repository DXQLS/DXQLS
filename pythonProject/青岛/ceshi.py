...
for i in range(8, 9):
    ...
    if not is_server_up(url_1):
        ...
        continue
    try:
        respones_1 = session.post(url=url_1, json=data_1, headers=headers, timeout=20).json()
        ...
        for data1 in datas_1:
            ...
            respones_2 = session.post(url=url_2, json=data_2, headers=headers, timeout=30).json()
            # datas_2.append(respones_2['data']['list'])
            datas_2 = respones_2['data']['list']
            all_datas_2.append(data_2)
            print(all_datas_2)
            # print(datas_1, datas_2)
            # print(datas_2)
            for item2 in datas_2:
                new_data = {**data1, **item2}
                # print(new_data)
...
# print(df_filtered)
df_filtered.to_excel('D:\pythonProject\pythonProject\青岛\青岛药品_自制药集采.xlsx', index=False)
print('数据已写入表格，共计' + format(len(df_filtered)))
